...
export default function YellowHandCue({
  sign,
  variant = "thumb",
  showText = true,
  activeFrame = null,
  ...
  const sourceFrames = cue.frames.length ? cue.frames : defaultCue.frames;
  const frames = variant === "thumb"
    ? [sourceFrames[sourceFrames.length - 1]]
    : activeFrame === null
      ? sourceFrames
      : [sourceFrames[activeFrame % sourceFrames.length]];

  return (
    <div className={`yellow-hand-cue ${variant} ${animate ? "animating" : ""} ${speed === "slow" ? "slow" : ""}`}>
